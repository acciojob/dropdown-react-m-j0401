...
export function getCity(state) {
  ...
  return st ? st.city : [];   // ✅ always array
}

export function getLandmark(state, city) {
  const st = states.find(s => s.name === state);
  if (!st) return [];

  const ct = st.city.find(c => c.name === city);
  return ct ? ct.landmarks : [];  // ✅ always array
}
//console.log(getLandmark("Madhya Pradesh","Gwalior"));
